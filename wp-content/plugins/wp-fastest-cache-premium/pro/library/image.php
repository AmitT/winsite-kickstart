<?php
	class WpFastestCacheImageOptimisation{
			...
			if(!$this->backup_folder_exist()){
				...
				foreach ($this->images as $key => $value) {
					...
					if($res["success"]){
						...
						if(in_array($res["error_code"] , array(2, 6, 7, 11))){
		...
		public function compress_image_external($url){
			...
			$response = wp_remote_get("https://api.wpfastestcache.net/image/compress/".$url, 
		...
		public function setImages(){
			if(isset($this->metadata["file"]) && $this->metadata["file"]){
				...
				foreach ((array)$this->metadata["sizes"] as $key => $value) {
					...
					if($i == 0){
						if($this->is_url_404($this->uploadDir['baseurl']."/".$this->metadata["file"])){
							$image_error = true;
							break;
						}
					}

					if(!$this->is_url_404($value["url"]) && $this->acceptable_mime_type($value["file"])){
		...
		public function statics(){
			include_once($this->get_template_path("statics.html"));
		}
